...
@section('content')
  <div class="col-md-3 pb-4">
    @component('components.sidebar-partai-member', ['type' => 'organizations', 'partaiMemberId' => $partaiMember['id']])
    @endcomponent
  </div>
  <div class="col-lg-9">
    <div class="card custom-card overflow-hidden">
      <div class="card-header d-flex justify-content-between">
        <h5><b>Organisasi</b></h5>
        <a class="btn btn-primary my-2 btn-icon-text" href="{{ route('backend.partai-member.organizations.create', $partaiMember['id']) }}">
  ...
  <script>
    $(document).ready(function () {
      ...
      let dataTable = $('#Datatable').DataTable({
        ...
        pageLength: 10,
        ajax: "{{ route('backend.partai-member.organizations.index', $partaiMember['id']) }}",
        columns: [
          {data: 'name', name: 'name'},
          {data: 'year', name: 'year'},
      ...
      modalDelete.addEventListener('show.bs.modal', function (event) {
        ...
        this.querySelector('.urlDelete').setAttribute('href', '{{ route("backend.partai-member.organizations.index", $partaiMember['id']) }}/' + id);
      });
