...
class ProfessionController extends Controller
{
  ...
  public function index(PartaiMember $partaiMember, Request $request)
  {
    ...
    if ($request->ajax()) {
      $data = Profession::where('partai_member_id', $partaiMember['id']);
        ...
        ->addColumn('action', function ($row) use ($partaiMember) {
          $actionBtn = '<div class="dropdown">
                          ...
                          <ul class="dropdown-menu">
                            <li><a href="' . route('backend.partai-member.professions.edit', ['partai_member' => $partaiMember['id'], $row['id']]) . '" class="dropdown-item">Ubah</a></li>
    ...
    return view('backend.professions.index', compact('page_breadcrumbs', 'config', 'partaiMember'));
  }
  ...
  public function create(PartaiMember $partaiMember)
  {
    ...
    return view('backend.professions.create', compact('page_breadcrumbs', 'config', 'partaiMember'));
  }
  ...
  public function store(PartaiMember $partaiMember, Request $request)
  {
    ...
    if ($validator->passes()) {
      ...
      $response = $this->responseStore(true, route('backend.partai-member.professions.index', $partaiMember['id']));
    } else {
  ...
  public function edit(PartaiMember $partaiMember, $id)
  {
    ...
    return view('backend.professions.edit', compact('page_breadcrumbs', 'config', 'partaiMember', 'data'));
  }
  ...
  public function update(PartaiMember $partaiMember, Request $request, $id)
  {
    ...
    if ($validator->passes()) {
      ...
      $response = $this->responseUpdate(true, route('backend.partai-member.professions.index', $partaiMember['id']));
    } else {
      $response = response()->json(['error' => $validator->errors()->all()]);
    }
    return $response;
