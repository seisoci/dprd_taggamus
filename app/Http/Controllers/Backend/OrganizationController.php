...
class OrganizationController extends Controller
{
  ...
  public function index(PartaiMember $partaiMember, Request $request)
  {
    ...
    $page_breadcrumbs = [
      ...
      ['url' => '#', 'title' => "Organisasi"],
    ];

    if ($request->ajax()) {
      $data = Organization::where('partai_member_id', $partaiMember['id']);
        ...
        ->addColumn('action', function ($row) use ($partaiMember) {
          $actionBtn = '<div class="dropdown">
                          ...
                          <ul class="dropdown-menu">
                            <li><a href="' . route('backend.partai-member.organizations.edit', ['partai_member' => $partaiMember['id'], $row['id']]) . '" class="dropdown-item">Ubah</a></li>
    ...
    return view('backend.organizations.index', compact('page_breadcrumbs', 'config', 'partaiMember'));
  }
  ...
  public function create(PartaiMember $partaiMember)
  {
    ...
    $page_breadcrumbs = [
      ['url' => route('backend.partai-member.index'), 'title' => "Anggota Partai"],
      ['url' => route('backend.partai-member.show', $partaiMember), 'title' => "Detail Anggota Partai"],
      ['url' => '#', 'title' => "Tambah Pekerjaan"],
    ];

    return view('backend.organizations.create', compact('page_breadcrumbs', 'config', 'partaiMember'));
  }
  ...
  public function store(PartaiMember $partaiMember, Request $request)
  {
    ...
    if ($validator->passes()) {
      ...
      $response = $this->responseStore(true, route('backend.partai-member.organizations.index', $partaiMember['id']));
    } else {
  ...
  public function edit(PartaiMember $partaiMember, $id)
  {
    ...
    $page_breadcrumbs = [
      ['url' => route('backend.partai-member.index'), 'title' => "Anggota Partai"],
      ['url' => route('backend.partai-member.show', $partaiMember), 'title' => "Detail Anggota Partai"],
      ['url' => '#', 'title' => "Edit Pekerjaan"],
    ];

    $data = Organization::find($id);

    return view('backend.organizations.edit', compact('page_breadcrumbs', 'config', 'partaiMember', 'data'));
  }
  ...
  public function update(PartaiMember $partaiMember, Request $request, $id)
  {
    ...
    if ($validator->passes()) {
      ...
      $response = $this->responseUpdate(true, route('backend.partai-member.organizations.index', $partaiMember['id']));
    } else {
      $response = response()->json(['error' => $validator->errors()->all()]);
    }
    return $response;
